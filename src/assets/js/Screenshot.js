...
OI.ready(function(){
	...
	function Screenshot(el){
		...
		el.addEventListener('click',(e) => {
			// Arrow function uses the external 'this'
			var opts = { "original": el, "file": "figure.png" };
			if(this.hide) opts.hide = this.hide;
			saveDOMImage(document.querySelector(this.target), opts);
		});
		return this;
	}
	
	var buttons = document.querySelectorAll('[data-comp=Screenshot]');
	var bs = new Array(buttons.length);
	for(var i = 0; i < buttons.length; i++) bs[i] = new Screenshot(buttons[i]);
